...
def IUPred2A(pro_uip, method_path, iupred2a):
    ...
    out_long_dir = os.path.join('./example/tmp/Protein_IUPred2A_long.txt')
    out_short_dir = os.path.join('./example/tmp/Protein_IUPred2A_short.txt')
    out_glob_dir = os.path.join('./example/tmp/Protein_IUPred2A_glob.txt')
...
def ProFeature(prolist, pro_uip, method_path, protbert,
    ...
    #x_dense_p = protein_dense_feature_dict(prolist, pro_uip, method_path, iupred2a, ncbiblast, nrdb90)
    x_bert_p = {}
    for tmp in range(len(prolist)):
        bert_p = PreTrain(prolist[tmp], pro_uip, 800, method_path, protbert, device)
        x_bert_p[prolist[tmp]] = bert_p
    output_file = 'example/test_x_bert_p.pkl'
    with open(output_file, 'wb') as f:
        pickle.dump(x_bert_p, f)
    # x_bert_p = PreTrain(prolist, pro_uip, 800, method_path, protbert, device)
    # return x_p.to(device), x_2_p.to(device), x_dense_p.to(device), x_bert_p.to(device)
    return x_p, x_2_p, x_bert_p
...
if __name__ == '__main__':
    ...
    x_p, x_2_p, x_bert_p = ProFeature(pro_seq_list, pro_uip, method_path, protbert, iupred2a, ncbiblast, nrdb90, device)