...
global BLAST
global BLAST_DB
BLAST = os.path.join('ncbi-blast/ncbi-blast-2.13.0+/', 'bin/psiblast.exe')
BLAST_DB = os.path.join('IIDL-PepPI/utils/nrdb90/', 'nrdb90')
...
def generateMSA(file_path):
    seq_DIR = [file_path.rsplit('/', 1)[-1]] #example/
    pool = Pool(1)
...
def get_protein_blosum(method_path, proteinseq):
    blosum62 = {}
    blosum_reader = open(os.path.join('./utils/blosum62.txt'), 'r')
    count = 0
...
def PSSM(method_path, ncbiblast, nrdb90, pro_uip):
    BLAST = os.path.join(ncbiblast, 'bin/psiblast.exe')
