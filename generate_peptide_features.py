...
def IUPred2A(pep_uip, method_path, iupred2a):
    ...
    out_long_dir = os.path.join('./example/tmp/Peptide_IUPred2A_long.txt')
    out_short_dir = os.path.join('./example/tmp/Peptide_IUPred2A_short.txt')
    out_glob_dir = os.path.join('./example/tmp/Peptide_IUPred2A_glob.txt')
...
def PepFeature(peplist, pep_uip, method_path, protbert, iupred2a, ncbiblast, nrdb90, device):
    x_pep = peptide_feature_dict(peplist) #做一个氨基酸到id的映射
    # x_ss_pep = peptide_ss_feature_dict(peplist, pep_uip, method_path, scratch)
    x_2_pep = peptide_2_feature_dict(peplist)
    #获取每一条序列预训练嵌入
    # x_bert_pep = {}
    # for tmp in range(len(peplist)):
    #     bert_pep = PreTrain(peplist[tmp], pep_uip, 50, method_path, protbert, device)
    #     x_bert_pep[peplist[tmp]] = bert_pep
    # output_file = 'example/test_x_bert_pep.pkl'  #对应数据集路径
    # with open(output_file, 'wb') as f:
    #     pickle.dump(x_bert_pep, f)
    #x_bert_pep = PreTrain(peplist, pep_uip, 50, method_path, protbert, device)
    # x_diso_pep = peptide_dense_feature_dict(peplist, pep_uip, method_path, iupred2a)
    # return x_pep.to(device), x_2_pep.to(device), x_diso_pep.to(device), x_bert_pep.to(device)
    return x_pep, x_2_pep
...
if __name__ == '__main__':
    ...
    parser.add_argument('-pep_fasta', type=str, default='example/test_peptide.fasta',
                        help='Fasta file path of peptide sequence')
    parser.add_argument('-pro_fasta', type=str, default='example/test_protein.fasta',
    ...
    x_pep, x_2_pep, x_bert_pep = PepFeature(pep_seq_list, pep_uip, method_path, protbert, iupred2a, ncbiblast, nrdb90, device)